...
class PfDemoCtrl(pyv.EvListener):
    ...
    def on_quit(self, ev):
        print('cross-->envoi event gameover')
    ...
    def on_keydown(self, ev):
        if ev.key == pyv.keycodes.K_ESCAPE:
            print('key-->envoi event gameover')
            self.pev(pyv.EngineEvTypes.Gameover)

        elif ev.key == pyv.keycodes.K_BACKSPACE:
            self._m.last_res = None

        elif ev.key == pyv.keycodes.K_RETURN:
            print(DEBUG_MSG)
        ...
        elif ev.key == pyv.keycodes.K_LEFT:
